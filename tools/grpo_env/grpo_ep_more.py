...
MAXM = int(os.environ.get("GRPO_MAXM", "8"))   # <more/> continuations per episode
...
def serve(kw, ask):
    ...
    return tokenizer.decode(ids[:PAGE_STEP]), "(no extraction)", pg, ids
...
# and until now nothing came back. MORE fires only at the very end of the generated
# text, so one tag is served exactly once.
MORE = re.compile(r"<\s*/?\s*more\s*/?\s*>\s*$", re.I)
...
def episode(prompt_ids, question, gold):
    ...
    while n_gen < GEN:
        ...
        if mclose and ns == len(queries_out) and txt.count("<search>") > ns:
            ...
            if not kw:
                ...
                page_ids, page_off = pids, PAGE_STEP
                blk = f"\n<information>\n{chunk}\n[READER] {span}\n</information>\n"
    ...
    return ids, mask, (ns, grounded, landed, correct, bridge, nm), \
        {"queries": queries_out, "answer": (ans if landed else ""), "hints": hints[:5],
         "more": nm, "text": tokenizer.decode(ids[len(prompt_ids):])}


def price(group):
    """Grounded-correct is 1.0. Everything else is 0.0. No partial credit, under any condition."""
    return [1.0 if (g and c) else 0.0 for _, g, _, c, _, _ in group]
...
for step in range(1, STEPS + 1):
    ...
    for b in range(B):
        ...
        for (_, _, inf, trace) in eps:
            roll_fh.write(json.dumps({
                ...
                "correct": inf[3], "trusted": inf[4], "more": inf[5],
    ...
    line = (f"[step {step}] correct={corr:.0%} grounded={gnd:.0%} landed={land:.0%} "
            ...
            f"morerate={sum(1 for i in step_info if i[5])/n:.0%} |grad|={gnorm:.4f} "

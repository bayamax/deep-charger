...
{ [ -s /root/work/greedy_ans_out_0.jsonl ] && HF_TOKEN=$(tr -d '[:space:]' < /root/.hf_token) hf upload baya1116/hypernet-sp-distill /root/work/greedy_ans_out_0.jsonl pooler_distill/chatsft/rollouts/greedy_ans_partial_0.jsonl 2>&1 | tail -1; echo "PEEK greedy_ans $(wc -l < /root/work/greedy_ans_out_0.jsonl 2>/dev/null) rollouts $(date -u +%H:%M)"; } >> /proc/1/fd/1 2>&1
exit 0

...
MODE=sft2
SRUN2=s4
SDATA=pooler_distill/chatsft/mix_v3.jsonl
S2REPLAY=0
S2RANK=16
S2LR=5e-5
S2EPOCHS=3
...
PYR
  ...
  if [ "$RUNNING" = "0" ]; then
    ...
    cd /root/work && PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True setsid nohup python3 /root/work/sft_lora.py \
      ...
      --rank ${S2RANK:-16} --lr ${S2LR:-3e-5} --epochs ${S2EPOCHS:-2} --accum ${S2ACCUM:-8} --val ${S2VAL:-12} --patience ${S2PATIENCE:-0} --maxlen ${S2MAXLEN:-3072} \

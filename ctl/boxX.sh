...
GN=9999
GPAR=1
GQFILE=pooler_distill/selfq_0.jsonl
GMODEL=s4_hf
GSTOP=eos
GTEMP=0.6
SHARDS=3
...
if [ "$MODE" = "gen" ]; then
  ...
  GMODEL=${GMODEL:-}   # a merged model directory under pooler_distill/chatsft on the hub; empty = the step-200 student
  GBASE=/root/eval_hf200
  if [ -n "$GMODEL" ]; then
    GBASE=/root/hfdl/pooler_distill/chatsft/$GMODEL
    for try in 1 2 3 4 5 6; do hf download $R --include "pooler_distill/chatsft/${GMODEL}/*" --local-dir /root/hfdl >/dev/null 2>&1; [ -s $GBASE/model.safetensors ] && break; sleep 30; done
    [ -s $GBASE/model.safetensors ] || { echo "GEN_ABORT: $GMODEL not on the hub"; exit 0; }
  fi
  GSTOP=${GSTOP:-answer}
...
#!/bin/bash
GRUN=$GRUN; GTEMP=$GTEMP; R=$R; GPAR=$GPAR; GBASE=$GBASE; GSTOP=$GSTOP
GKQ

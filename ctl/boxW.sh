...
MODE=sweep
SWMODEL=s4_hf
SWEEPS="p04:0.4:plain p05:0.5:plain g05:0.5:guard g06:0.6:guard"
SHARDS=3
...
if [ "$MODE" = "sweep" ]; then
  # Sampling settings for the no-search replies: the same 60 prompts through one fine-tuned model
  # under several temperature / decode-guard combinations; the replies are judged off-box.
  pkill -f "afterkee[p].sh"; pkill -f "evalkee[p].sh"; pkill -f "genkee[p].sh"; pkill -f "sft2kee[p].sh"; pkill -f "reevalkee[p].sh"; pkill -f "sweepkee[p].sh"; sleep 2
  pkill -f "pool_eval.p[y]"; sleep 8; pkill -9 -f "pool_eval.p[y]" 2>/dev/null; sleep 2
  export HF_TOKEN=$(tr -d '[:space:]' < /root/.hf_token 2>/dev/null)
  R=baya1116/hypernet-sp-distill
  SWHF=/root/hfdl/pooler_distill/chatsft/$SWMODEL
  for try in 1 2 3 4 5 6; do hf download $R --include "pooler_distill/chatsft/${SWMODEL}/*" --local-dir /root/hfdl >/dev/null 2>&1; [ -s $SWHF/model.safetensors ] && break; sleep 30; done
  [ -s $SWHF/model.safetensors ] || { echo "SWEEP_ABORT: $SWMODEL not on the hub"; exit 0; }
  ...
  cat > /root/sweepkeep.sh <<SWK
#!/bin/bash
SWHF=$SWHF; R=$R; SWEEPS="$SWEEPS"; SWMODEL=$SWMODEL
SWK
  cat >> /root/sweepkeep.sh <<'SWKB'
export HF_TOKEN=$(tr -d '[:space:]' < /root/.hf_token 2>/dev/null)
for spec in $SWEEPS; do
  name=${spec%%:*}; rest=${spec#*:}; temp=${rest%%:*}; dec=${rest#*:}
  out=/root/work/sweep_${name}.jsonl
  if [ ! -s $out ] || [ "$(wc -l < $out)" -lt 60 ]; then
    cd /root/work && SP_BASE=$SWHF SP_RANK=16 SP_NOSYS=1 SP_EPISODIC=1 OMP_NUM_THREADS=1 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python3 /root/work/pool_eval.py       /root/pooler200.safetensors /root/hfdl/pooler_distill/chat_eval60.jsonl $out --n 999 --rw 768 --maxd 384 --samepage 1 --decode $dec --temp $temp --stop eos --replycap 200 --tag "[sw_$name]" >> /root/sweep_$name.log 2>&1
  fi
  hf upload $R $out pooler_distill/chatsft/rollouts/sweep_${SWMODEL}_${name}.jsonl >/dev/null 2>&1
  echo "[sweep] $name done: $(tail -1 /root/sweep_$name.log | cut -c1-110)"
done
echo "SWEEP_DONE $SWMODEL $(date -u)"
SWKB
  chmod +x /root/sweepkeep.sh; setsid nohup bash /root/sweepkeep.sh >> /proc/1/fd/1 2>&1 < /dev/null &
  sleep 5; echo "SWEEP_LAUNCH_DONE $SWMODEL $(date -u)"; exit 0
fi

...
RSHARDS="0"
RN=10   # both rules on this one card, 10 questions first; widen only if they differ or agree too neatly
SHARDS=3
...
#!/bin/bash
RRUN=$RRUN; RHF=$RHF; R=$R; RSTOP=${RSTOP:-eos}; RTEMP=${RTEMP:-0.9}; RGREEDY=${RGREEDY:-0}; RRUNS="${RRUNS:-}"; RSHARDS="${RSHARDS:-0 1 2}"; RN=${RN:-999}
RK
  cat >> /root/reevalkeep.sh <<'RKB'
export HF_TOKEN=$(tr -d '[:space:]' < /root/.hf_token 2>/dev/null)
run_one() {  # $1 questions file, $2 out file, $3 tag
  want=$(wc -l < "$1"); [ "$RN" -lt "$want" ] && want=$RN
  [ -s "$2" ] && [ "$(wc -l < "$2")" -ge "$want" ] && return 0
  cd /root/work && SP_BASE=$RHF SP_RANK=16 SP_NOSYS=1 SP_EPISODIC=1 OMP_NUM_THREADS=1 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python3 /root/work/pool_eval.py     /root/pooler200.safetensors "$1" "$2" --n $RN --rw 768 --maxd 384 --samepage 1 --decode plain --temp ${RTEMP:-0.9} --greedy ${RGREEDY:-0} --stop ${RSTOP:-eos} --replycap 200 --tag "[$3]" >> /root/${RRUN}_$3.log 2>&1
